    ...
    return (
        <div style={{ display: 'flex', gap: '16px', padding: '16px', overflowX: 'auto' }}>
          {Object.entries(groupedForecast).map(([date, entries], index) => (
            ...
            >
              <h3 style={{ fontSize: '1.5rem', margin: '8px 0' }}>{getDayOfWeek(date)}</h3>
              {selectedDay === date ? (
                <div style={{ display: 'flex', flexDirection: 'row', gap: '16px', marginTop: '16px' }}>
                  {entries.map((entry, entryIndex) => (
                    ...
                    >
                      <p style={{ fontSize: '1rem', color: '#555' }}>{entry.dt_txt.split(' ')[1]}</p>
                      <img
                        src={`https://openweathermap.org/img/wn/${entry.weather[0].icon}.png`}
                        alt="weather icon"
                        style={{ width: '50px', height: '50px' }}
                      />
                      <p style={{ fontSize: '1rem', color: '#555' }}>{entry.weather[0].description}</p>
                      <p style={{ fontSize: '1.5rem', fontWeight: 'bold', margin: '8px 0' }}>{Math.round(entry.main.temp)} °C</p>
                ...
                <div style={{ textAlign: 'center' }}>
                  <img
                    ...
                    style={{ width: '50px', height: '50px' }}
                  />
                  <p style={{ fontSize: '1rem', color: '#555' }}>{entries[0].weather[0].description}</p>
                  <p style={{ fontSize: '1.5rem', fontWeight: 'bold', margin: '8px 0' }}>{Math.round(entries[0].main.temp)} °C</p>
                </div>
              )}
            </div>
          ))}
